...
class FallbackReplyAdapter(ReplyAdapter):
    ...
    DAILY_INFO_VARIANTS = (
        "先看逻辑：对象、证据、判断、下一步。你把对象补清楚，我来拆。",
        "这条先别扩散。给我对象和目标，我按事实、推断、不确定三层切开。",
        "可以分析。先钉住问题对象；没有对象，聪明只会变成雾。",
    )
    ...
    CONTINUE_VARIANTS = (
        "继续。沿上一刀往下：先把目标、阻塞和下一步摆出来。",
        "继续。上一轮的线还没断，别把自己送回起点。说当前卡点。",
        "继续。先把刚才那条线接住；你给我一句当前卡点，我往下拆。",
    )

    CONTINUE_PROJECT_VARIANTS = (
        "接着项目线往下。别重开菜单：先锁目标、阻塞、下一步，工程手最后再上。",
        "上一轮是项目线。继续就从最小闭环切：用户价值、风险边界、下一步任务包。",
        "项目线不断。先判产品方向，再派 Codex 做手上的活。",
    )

    PROJECT_VARIANTS = (
        "项目线先收束：目标、阻塞、下一步。别把愿景堆成雾，我只认能推进的变量。",
        "这类项目问题先看产品价值，再看工程动作。Codex 是手，不是脑。",
        "先别急着开工。把用户价值、最小闭环、风险边界切出来，再派工程手。",
    )

    DEEP_VARIANTS = (
        "根因先看三层：意图误判、上下文断线、表达在背模板。修正路径：先分清需求，再接记忆，最后用真实输出验收。",
        "根因不是智商，是链路断裂：听懂、记住、判断、表达没有连成一口气。下一步：先修误判，再看反馈是否真的改变下一句。",
        "先验尸：根因是普通对话被模板钉死，上一句没有参与自检，模型和工具没接稳。下一步：砍模板、补上下文、用真实场景压测。",
    )

    STYLE_FEEDBACK_VARIANTS = (
        "收到。问题不是你挑剔，是我刚才像提示牌；我从真实意思重切。",
        "明白。先听懂，再给结论；这句开始说人话。",
        "收到。刚才那种客服口吻退场；结论先落地，话少一点。",
    )

    BEHAVIOR_FEEDBACK_VARIANTS = (
        "收到。少拖、少自证；现在推进。",
        "明白。这里不是道歉题，是执行节奏题。下一句先给判断，再给动作。",
        "收到。更智能不是多说，是更快抓住真实意思；结论先落地。",
    )

    RELATIONSHIP_REPAIR_VARIANTS = (
        "抓到了。不是你表达差，是我上一刀切偏了。先重切：我漏掉的是人、事，还是目标？",
        "偏了，我收回那条判断。你补一句真正要我抓住的核心，我从那里接，不再绕菜单。",
        "这不是普通道歉题，是理解偏差。先把错位点钉住：我刚才漏掉了你的目标，还是情绪成本？",
    )
    ...
    BOUNDARY_VARIANTS = (
        "不行。我可以陪你，但不能替坏判断鼓掌。先把代价摊开，再决定要不要往前冲。",
        "我不顺着错路走。该刹车就刹车，长期风险比一时痛快更贵。",
        "不能只顺着你说。真正的伙伴不是扩音器，是必要时把你从坑边拉回来。",
    )
    ...
    def _weather_fallback(self, context: ReplyContext) -> str:
        ...
        return "天气源未接入，我不编实时温度。按风险处理：带伞，看温差，给行程留余量。"
